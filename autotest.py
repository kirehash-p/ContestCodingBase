...
def run_and_compare(folder_path, run_command):
    ...
    for i in range(testcases):
        with open(f'{folder_path}/input_{i+1}.txt', 'r') as f:
            input_data = f.read()
        with open(f'{folder_path}/output_{i+1}.txt', 'r') as f:
        ...
        if process.stdout.decode() != expected_output:
            print(f'Failed on test case {i+1}')
            print('Input:')
            print(input_data)
            ...
            print(f'Passed on test case {i+1}')
...
def build_and_run(file_or_folder_path):
    ...
    file_name = os.path.splitext(os.path.basename(file_or_folder_path))[0]
    ...
    for file in os.listdir(folder_path):
        if file.startswith(f'{file_name}.'):
            if file.endswith('.cpp') or file.endswith('.cc'):
                ...
                continue
            # テストケースを実行
            print(f'Running test cases ...', end='')
